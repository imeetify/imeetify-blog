...
export function BlogHeader() {
  ...
  return <>
    {announcementOpen && <div className="announcement-banner border-b">
      <div className="announcement-copy mx-auto flex max-w-[1260px] flex-wrap items-center justify-between gap-x-4 gap-y-2 px-5 py-2.5 text-[12px] font-semibold lg:px-8">
        <p className="min-w-0 flex-1 leading-5"><span className="mr-2 announcement-pill rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em]">New</span> Make every meeting count with imeetify.</p>
        <div className="flex shrink-0 items-center gap-3"><a className="announcement-link inline-flex items-center gap-1 transition-colors hover:opacity-75" href="https://imeetify.com" target="_blank" rel="noreferrer">Explore imeetify <ArrowUpRight aria-hidden="true" className="size-3.5" /></a><button type="button" className="inline-flex size-7 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-card hover:text-foreground" aria-label="Dismiss announcement" onClick={() => setAnnouncementOpen(false)}><X aria-hidden="true" className="size-4" /></button></div>
      </div>
    </div>}
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1260px] items-center justify-between px-5 py-4 lg:px-8">
        <Link href="/" className="shrink-0" onClick={() => setOpen(false)} aria-label="imeetify blog home"><Image src={logo} alt="imeetify" width={130} height={29} className="" unoptimized priority /></Link>
