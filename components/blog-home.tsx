...
export function BlogHome({ posts, recent, page, totalPages }: { posts: WpPost[]; recent: WpPost[]; page: number; totalPages: number }) {
  ...
  return <>
    ...
    <main className="mx-auto grid max-w-[1260px] gap-12 px-5 pb-16 lg:grid-cols-[minmax(0,7fr)_minmax(280px,3fr)] lg:gap-12 xl:gap-16 lg:px-8 lg:pb-24"><div className="flex min-w-0 flex-col gap-8">{featured && <BlogCard post={featured} featured />}<div className="grid gap-7 md:grid-cols-2">{rest.map((post) => <BlogCard key={post.id} post={post} />)}</div><NewsletterCta />{totalPages > 1 && <nav className="flex items-center justify-between border-t border-[#e3e7ed] pt-6" aria-label="Pagination"><span className="text-sm text-[#647087]">Page {page} of {totalPages}</span><div className="flex gap-2">{page > 1 && <Link className="inline-flex items-center gap-2 rounded-lg border border-[#dce2e9] px-4 py-2 text-sm font-bold text-[#17253d] hover:border-[#54bd70] hover:text-[#54bd70]" href={`/?page=${page - 1}`}><ArrowLeft aria-hidden="true" className="size-4" /> Previous</Link>}{page < totalPages && <Link className="inline-flex items-center gap-2 rounded-lg border border-[#dce2e9] px-4 py-2 text-sm font-bold text-[#17253d] hover:border-[#54bd70] hover:text-[#54bd70]" href={`/?page=${page + 1}`}>Next <ArrowRight aria-hidden="true" className="size-4" /></Link>}</div></nav>}</div><div className="min-w-0 self-start lg:sticky lg:top-[100px]"><BlogSidebar recent={recent} /></div></main>
  </>
}
