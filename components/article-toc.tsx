...
export function ArticleToc({ items }: { items: string[] }) {
  ...
  return <details className="group rounded-xl border border-[#e3e7ed] bg-white p-5" open><summary className="cursor-pointer list-none text-[11px] font-bold uppercase tracking-[0.14em] text-[#54bd70]">In this article <span className="float-right text-[#8994a6] transition group-open:rotate-180" aria-hidden="true">⌄</span></summary><nav aria-label="Table of contents"><ol className="mt-4 flex flex-col gap-3">{items.slice(0, 8).map((item, index) => <li key={`${item}-${index}`}><Link href={`#section-${index + 1}`} aria-current={active === index ? 'location' : undefined} className={`flex gap-3 border-l-2 pl-3 text-sm leading-5 transition ${active === index ? 'border-[#54bd70] font-semibold text-[#17253d]' : 'border-transparent text-[#647087] hover:text-[#54bd70]'}`}><span className="font-bold text-[#54bd70]">{String(index + 1).padStart(2, '0')}</span><span>{item}</span></Link></li>)}</ol></nav></details>
}
